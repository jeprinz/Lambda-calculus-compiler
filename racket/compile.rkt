  ...
  (match e
    ...
    [(Cclosure label) ;; create closure (label, current data) in heap, push pointer to closure onto stack
     (seq
      ...
      (Push heap-reg)
      (Add heap-reg 16))]
    [(Cvar i) ;; travel down n pointers to nth element of data in arg-register. push to stack.
     (let ((lstart (gensym))
           (lend (gensym)))
       (seq
    ...
    [(Ccall c1 c2) ;; Run c1, Run c2, results in stack, pop c2 to arg-register, call c1
     (seq
